...
| 1           | Loss of communications between server and PLC.                               | 1. Lift goes back to manual mode. <br/> 2. PLC state goes to error state.                                       | When the heartbeat is heard again, it goes back to human mode. No need for human intervention.                 |
...
| 6           | Mode Timeout (10 minutes)(Lift is unable to go to AGV mode from human mode)  | 1. Lift goes back to manual mode. <br/>2. PLC state goes to error state.                                        | Manual intervention is needed. PLC will stay in error state until it is restarted.                             |
